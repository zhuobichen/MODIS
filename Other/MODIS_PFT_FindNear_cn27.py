...
grid_nc  = "GRIDCRO2D_2000121_GuangDongD1" # CMAQ 网格（.nc 或无后缀均可）
YEAR     = 2000
hlines  = [23,24,25,26,27,28,29,30]               # 横向瓦片
vlines  = [3,4,5,6,7,8]                           # 纵向瓦片
exclude_tiles = set()                      # 例如 {"h30v06"}
SIDE_M   = 27000.0                          # 正方形边长（米）
HALF_M   = SIDE_M / 2.0
RADIUS   = HALF_M * math.sqrt(2.0)         # 圆半径（先圈选，再做方形过滤）
BATCH    = 6000                          # 每批处理的 CMAQ 点数
PARALLEL = True                           # 是否并行（默认否）
N_PROCS  = max(1, min(cpu_count(), 8))     # 并行进程数（启用并行时使用）
out_nc   = f"PFT_frac_{YEAR}_27km_square.nc"
out_csv  = f"PFT_frac_{YEAR}_27km_square.csv"

# 👉 要跳过的瓦片列表（输入格式形如 "h30v06"）
skip_tiles = ["h30v03","h29v03","h28v03","h27v03",
              "h30v04","h29v04",
