...
# 生成CELL_ID、X、Y坐标（核心修改：X和Y从1开始计数）
cell_ids = np.arange(1, total_pixels + 1)  # CELL_ID从1开始
# 原代码：y_coords, x_coords = np.mgrid[0:row, 0:col]  # 0-based索引
y_coords, x_coords = np.mgrid[1:row+1, 1:col+1]  # 修改为1-based索引（从1开始）
x_flat = x_coords.flatten(order='C')  # X（列）从1到col
y_flat = y_coords.flatten(order='C')  # Y（行）从1到row
